...
class FGPU:
    ...
    def set_paramerter(self, paramIndex, val):
        ...
        if paramIndex not in range(0,16):
            raise AssertionError()
        if(type(val)==int) or (type(val)==bytes) or (type(val)==float):
            self.params[paramIndex] = val
        else:
            self.params[paramIndex] = val.physical_address
    ...
    def compile_kernel(self, show_objdump=False, float_support=False):
        ...
        * Any mistakes in the OpenCL code will be shown if the clang-compilation was not successful
        ...
        if float_support:
            p= Popen([general_const.COMPILE_SH, self.kernelFileName, '-hard-float'], stdout=PIPE, stderr=PIPE)
        else:
